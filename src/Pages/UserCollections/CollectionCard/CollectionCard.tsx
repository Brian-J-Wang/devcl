...
const TaskDocCard: React.FC<TaskDocProps> = ({ taskDoc, handleDelete }) => {
		...
		<div className={`collection-card ${hidden ? "" : "collection-card_extended"}`}>
			<Container className="collection-card__header">
				<div className="collection-card__left">
					<button
						className={`collection-card__left-drop ${!hidden && "collection-card__left-drop_active"}`}
						onClick={() => {
							setHidden(!hidden);
						}}>
						<img src={arrow} alt="" />
					</button>
					<p className="collection-card__title">{taskDoc.name}</p>
			...
			<div className="content-panel">
				<div className="content-panel__tab-bar">
					<div
						className={`content-panel__tab ${activeTab == "Info" && "content-panel__tab_active"}`}
						onClick={() => setActiveTab("Info")}>
						Info
					</div>
					<div
						className={`content-panel__tab ${activeTab == "People" && "content-panel__tab_active"}`}
						onClick={() => setActiveTab("People")}>
						People
					</div>
					<div
						className={`content-panel__tab ${activeTab == "Settings" && "content-panel__tab_active"}`}
						onClick={() => setActiveTab("Settings")}>
						Settings
					</div>
				</div>
				<div hidden={activeTab != "People"} className="content-panel__people-tab"></div>
				<div hidden={activeTab != "Settings"} className="content-panel__setting-tab">
					...
					<div className="content-panel__setting">
						...
						<Input.Button style="negative" onClick={deleteCollection(taskDoc.id)}>
							Delete
						</Input.Button>
					</div>
				</div>
