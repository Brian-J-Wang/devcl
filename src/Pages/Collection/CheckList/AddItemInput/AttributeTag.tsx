...
const AttributeTag: React.FC<AttributeTagProps> = (props) => {
    return (
        <div style={{backgroundColor: `rgba(${stringToColour(props.attribute.key, 0.5)})`}} className={style.body} hidden={props.hidden ?? false}>
            {props.attribute.key} ⋮⋮ {props.attribute.value}
        </div>
    )
}
